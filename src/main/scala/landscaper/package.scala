...
package object landscaper {
  ...
  /** Transforms a data structure "d" using a function that is applied on all constituent of "d"
    ...
    * @return
    */
  def rewrite[FIn, FOut, In](f: FIn => FOut, d: In)(implicit func: Trans[FIn, FOut, In]): func.Result =
    func.rewrite(f, d)
  ...
  /** Constructs a pattern finder from a partial functions.
    ...
    *
    */
  def pattern[Out](pf: PartialFunction[Any, Seq[Out]]): PatternFinder.Aux[Any, Out] =
    PatternFinder(pf)
